...
public final class CacheManager {
    ...
    private static final String CACHE_DIR = "lotteryHttpCache";
    ...
    private CacheManager() {
        File diskCacheDir = new File(FileUtil.getCacheDir(MyApp.getAppContext()), CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(MyApp.getAppContext()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
