...
//        File toFile = SDPath.getFile("temp.jpg", SDPath.PHOTO_FILE_STR);

        ILogger.d("create folder=" + toFile.getAbsolutePath());
        if (suc) {
//            mTakePhotoUri = Uri.fromFile(toFile);
            mTakePhotoUri = FileProvider.getUriForFile(this,
//                    BuildConfig.APPLICATION_ID + ".provider",
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if ( requestCode == GalleryFinal.TAKE_REQUEST_CODE ) {
            if (resultCode == RESULT_OK && toFile != null) {
                final String path = toFile.getPath();
